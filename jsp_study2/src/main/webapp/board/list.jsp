...
<body class="p-3 mb-2 bg-secondary text-white">
	<h1 class="container-sm">Board List Page</h1>
	...
	<div class="container-sm">
		<!-- prev  -->
		<c:if test="${ph.prev }">
		 <a href="/brd/list?pageNo=${ph.startPage-1 }&qty=${ph.pgvo.qty}&type=${ph.pgvo.type}&keyword=${ph.pgvo.keyword}"><button type="button" class="btn btn-outline-dark"> ⇐ </button></a>
		 
		 </c:if>
		<!-- paging  -->
		<c:forEach begin="${ph.startPage }" end="${ph.endPage }" var="i">
			<a href="/brd/list?pageNo=${i }&qty=${ph.pgvo.qty}&type=${ph.pgvo.type}&keyword=${ph.pgvo.keyword}"><button type="button" class="btn btn-outline-dark"> ${i }</button></a>
			
		</c:forEach>
		<!-- next  -->
		<c:if test="${ph.next }">
		  <a href="/brd/list?pageNo=${ph.endPage+1 }&qty=${ph.pgvo.qty}&type=${ph.pgvo.type}&keyword=${ph.pgvo.keyword}"><button type="button" class="btn btn-outline-dark"> ⇒ </button></a>
